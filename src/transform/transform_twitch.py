"""
Transforms raw Twitch API payloads from raw_twitch table into the
content_items and engagement_snapshots tables.

Since Twitch has no public likes/comments data for videos
the columns are always stored as 0 for this platform.

Usage: python -m src.transform.transform_twitch
"""
import os
...
def transform_twitch(engine=None) -> int:
    engine = engine or create_engine(DATABASE_URL)
    count = 0

    with engine.begin() as conn:
        platform_account_id = get_platform_account_id(conn, "twitch")

        raw_rows = conn.execute(
            text("SELECT extracted_at, payload FROM raw_twitch")
        ).fetchall()
